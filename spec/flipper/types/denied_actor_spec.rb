...
RSpec.describe Flipper::Types::DeniedActor do
  subject do
  ...
  describe '.wrappable?' do
    it 'returns true if denied actor' do
      thing = thing_class.new('1')
      denied_actor = described_class.new(thing)
      expect(described_class.wrappable?(denied_actor)).to eq(true)
    end
  ...
  describe '.wrap' do
    context 'for denied actor' do
      it 'returns denied actor' do
        denied_actor = described_class.wrap(subject)
        expect(denied_actor).to be_instance_of(described_class)
        expect(denied_actor).to be(subject)
      end
    end
    ...
    context 'for other thing' do
      it 'returns denied actor' do
        thing = thing_class.new('1')
        denied_actor = described_class.wrap(thing)
        expect(denied_actor).to be_instance_of(described_class)
      end
    end
  end
  ...
  it 'initializes with thing that responds to id' do
    thing = thing_class.new('1')
    denied_actor = described_class.new(thing)
    expect(denied_actor.value).to eq('1')
  end
  ...
  it 'converts id to string' do
    thing = thing_class.new(2)
    denied_actor = described_class.new(thing)
    expect(denied_actor.value).to eq('2')
  end

  it 'proxies everything to thing' do
    thing = thing_class.new(10)
    denied_actor = described_class.new(thing)
    expect(denied_actor.admin?).to eq(true)
  end

  it 'exposes thing' do
    thing = thing_class.new(10)
    denied_actor = described_class.new(thing)
    expect(denied_actor.thing).to be(thing)
  end
  ...
  describe '#respond_to?' do
    it 'returns true if responds to method' do
      thing = thing_class.new('1')
      denied_actor = described_class.new(thing)
      expect(denied_actor.respond_to?(:value)).to eq(true)
    end

    it 'returns true if thing responds to method' do
      thing = thing_class.new(10)
      denied_actor = described_class.new(thing)
      expect(denied_actor.respond_to?(:admin?)).to eq(true)
    end

    it 'returns false if does not respond to method and thing does not respond to method' do
      thing = thing_class.new(10)
      denied_actor = described_class.new(thing)
      expect(denied_actor.respond_to?(:frankenstein)).to eq(false)
    end
  end
end
