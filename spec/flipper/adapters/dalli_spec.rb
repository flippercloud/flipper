...
RSpec.describe Flipper::Adapters::Dalli do
  ...
  describe '#get_all' do
    ...
    it 'warms all features' do
      adapter.get_all
      expect(DataStores.dalli.get(described_class.key_for(stats))[:boolean]).to eq('true')
      expect(DataStores.dalli.get(described_class.key_for(search))[:boolean]).to be(nil)
      expect(DataStores.dalli.get(described_class::GetAllKey)).to be_within(2).of(Time.now.to_i)
    end
